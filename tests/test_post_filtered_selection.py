...
def test_match_with_selection_completed(mocker: MockerFixture):
    """Test that match returns True for pipesv2_selection completion events."""
...
def test_match_with_wrong_pipeline(mocker: MockerFixture):
    """Test that match returns False for non-selection pipelines."""
    handler = PostFilteredSelectionHandler(
        monitoring_client=mocker.MagicMock(),
        bq_client=mocker.MagicMock(),
        run_project_id="test-project",
