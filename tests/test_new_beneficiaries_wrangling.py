"""Unit tests for NewBeneficiariesWranglingHandler."""
...
def test_match(mocker: MockerFixture, decoded_message, expected):
    ...
    handler = NewBeneficiariesWranglingHandler(
...
def test_handle_delegates_to_base(mocker: MockerFixture):
    """Test that handle delegates to _handle_new_beneficiaries_metrics with correct parameters."""
    handler = NewBeneficiariesWranglingHandler(
        monitoring_client=mocker.MagicMock(),
        bq_client=mocker.MagicMock(),
        run_project_id="test-project",
    ...
    mock_handle_new_beneficiaries_metrics = mocker.patch.object(
        handler, "_handle_new_beneficiaries_metrics"
    )
    handler.handle(decoded_message)

    # Verify it was called with the correct parameters
    mock_handle_new_beneficiaries_metrics.assert_called_once_with(
