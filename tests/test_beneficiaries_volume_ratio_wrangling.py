"""Unit tests for BeneficiariesVolumeRatioWranglingHandler."""
...
def test_match(mocker: MockerFixture, decoded_message, expected):
    ...
    handler = BeneficiariesVolumeRatioWranglingHandler(
...
def test_handle_delegates_to_base(mocker: MockerFixture):
    """Test that handle delegates to _handle_beneficiaries_volume_ratio_metrics with correct
    parameters."""
    handler = BeneficiariesVolumeRatioWranglingHandler(
        monitoring_client=mocker.MagicMock(),
        bq_client=mocker.MagicMock(),
    ...
    mock_handle_metrics = mocker.patch.object(handler, "_handle_beneficiaries_volume_ratio_metrics")
