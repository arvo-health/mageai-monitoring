"""Handler for calculating and emitting new providers metrics for wrangling pipeline.

This handler processes completion events from the pipesv2_wrangling pipeline
to compute metrics about new providers (providers not present in a 3-month
rolling history).
"""

from google.cloud import bigquery, monitoring_v3

from handlers.new_providers_base import NewProvidersBaseHandler


class NewProvidersWranglingHandler(NewProvidersBaseHandler):
    """
    Calculates and emits metrics for new providers from wrangling pipeline.
    ...
    def match(self, decoded_message: dict) -> bool:
        ...
        pipeline, which triggers the calculation of new providers metrics.
    ...
    def handle(self, decoded_message: dict) -> None:
        ...
        """
        self._handle_new_providers_metrics(
