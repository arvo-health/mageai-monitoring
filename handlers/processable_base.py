...
class ProcessableBaseHandler(Handler):
    ...
    def _handle_processable_metrics(
        ...
        query = f"""
        WITH processable_sum AS (
            SELECT COALESCE(SUM(vl_pago), 0) AS total_vl_pago
            FROM `{full_processable_table}`
        ),
        unprocessable_sum AS (
            SELECT COALESCE(SUM(vl_pago), 0) AS sum_unprocessable_vl_pago
            FROM `{full_unprocessable_table}`
        )
        SELECT
            p.total_vl_pago,
            u.sum_unprocessable_vl_pago,
            CASE
                WHEN (p.total_vl_pago + u.sum_unprocessable_vl_pago) > 0
                THEN p.total_vl_pago / (p.total_vl_pago + u.sum_unprocessable_vl_pago)
                ELSE 0
            END AS relative_vl_pago
        FROM processable_sum p, unprocessable_sum u
        """
