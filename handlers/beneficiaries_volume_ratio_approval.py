"""Handler for calculating and emitting beneficiaries volume ratio metrics for approval pipeline.

This handler processes completion events from the pipesv2_approval pipeline
to compute metrics about beneficiaries volume ratio.
"""

from google.cloud import bigquery, monitoring_v3

from handlers.beneficiaries_volume_ratio_base import BeneficiariesVolumeRatioBaseHandler


class BeneficiariesVolumeRatioApprovalHandler(BeneficiariesVolumeRatioBaseHandler):
    """
    Calculates and emits metrics for beneficiaries volume ratio from approval pipeline.

    When the pipesv2_approval pipeline completes, this handler queries BigQuery
    to calculate the ratio of the number of beneficiaries in the last 30 days,
    including the latest batch, to the number of beneficiaries in the previous 30 days period
    (D-60 to D-30). This enables monitoring of the volume of beneficiaries during
    ...
    def match(self, decoded_message: dict) -> bool:
        ...
        pipeline, which triggers the calculation of beneficiaries volume ratio metrics.
    ...
    def handle(self, decoded_message: dict) -> None:
        ...
        """
        self._handle_beneficiaries_volume_ratio_metrics(
